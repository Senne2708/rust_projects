...
impl World {
    ...
    fn gen_reward_cell(max: usize, snake_body: &Vec<SnakeCell>) -> Option<usize> {
        let mut reward_cell = rnd(max);

        // So the reward cell spawns outside the snake body
        loop {
            reward_cell = rnd(max);
    ...
    pub fn game_state(&self) -> Option<GameStatus> {
        self.status
    }
    ...
    pub fn game_status_text(&self) -> String {
        match self.status {
            ...
            Some(GameStatus::Played) => String::from("You are Playing!"),
